...
const FxHeader = () => {
  ...
  const containerVariants = {
    ...
    visible: {
      opacity: 1, y: 0,
      transition: { delay: 0.2, duration: 0.8, when: 'beforeChildren', staggerChildren: 0.3 },
    },
  }
  ...
  const imageVariants = {
    ...
    visible: { opacity: 1, scale: [1, 1.05, 1], transition: { duration: 1.5, ease: 'easeInOut' } },
  }
  ...
  return (
    <header className="relative fxheader flex items-center overflow-hidden pb-8" ref={ref}>
        ...
        <div className="flex md:justify-between md:flex-row flex-col md:items-center">
          <motion.div className="w-full md:w-1/2 text-center md:text-left pt-20 md:pt-0 z-30" variants={textVariants}>
            <motion.div className="space-y-8 pt-8">
              ...
              <motion.h1 className="z-50 bg-clip-text bg-gradient-to-r animate-pulse" variants={headingVariants}>
                <WhatsAppButton />
              </motion.h1>
            </motion.div>
          </motion.div>
          ...
          <div className="md:flex rounded-lg hidden md:items-center lg:justify-end w-full mt-8 md:w-1/2">
            <motion.div className="relative" variants={imageVariants}>
              <div className="w-full h-[18rem] lg:w-[35rem] object-cover md:h-[25rem] lg:h-[28rem]">
                <motion.video className="h-full w-full object-cover object-center rounded-lg shadow-lg" src={Cha2} alt="Trading Video" muted loop autoPlay playsInline />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <div className="animated-gradient-background absolute top-0 right-0 bottom-0 left-0 z-[-1] overflow-hidden">
        <motion.img
          className="w-full css-selector h-full object-cover opacity-30"
          src={Cha}
          alt="Background"
          initial={{ scale: 1 }}
          animate={{ scale: 1.2 }}
